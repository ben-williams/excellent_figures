...
<<cartesian,echo=TRUE, fig.height=3.9,fig.width=4, fig.align='center', fig.cap='Combine legends.',dev='png',dpi=300, fig.pos='H'>>=
...
<<xlimchange,echo=TRUE, fig.height=3.9,fig.width=4, fig.align='center', fig.cap='Combine legends.',dev='png',dpi=300, fig.pos='H'>>=
ggplot(mtcars, aes(wt, mpg))+
  geom_point(aes(size=hp, color=hp))+
  geom_smooth(alpha=.1)+
  scale_colour_gradientn(colours=rainbow(5), breaks=as.vector(bs))+
...
For this example (Figure 1) I have called for a map of the Bering Sea, with a zoom of ``4"
(zoom needs to be an integer from 3 (continent) to 21 (building); more info here).

<<ggmap, message=FALSE, fig.height=4, fig.cap='A map of the Bering Sea, pulled directly from Google Maps via ggmap.'>>=
library(ggmap)
...
A map can be adjusted (centered) on a specific location (Figure 2), note that I changed
...
First call the map.
<<ggmap1, message=FALSE, fig.height=4, fig.cap='A map of the Bering Sea, pulled directly from Google Maps via ggmap.'>>=
bs1<-get_map(location = c(-170,63), zoom = 6)
@

Next add points to the map.
<<ggmap2, message=FALSE, fig.height=4, fig.cap='A map of the Bering Sea, pulled directly from Google Maps via ggmap.'>>=
...
Figure 2: A slightly zoomed in map of the Bering Sea, centered on specific coordinate
locations.


Polygons can be easily added in a manner similar to adding point locations. Here I've simply
made a polygon from the made-up database (Figure 3). You would have a separate database
...
Figure 3: A slightly zoomed in map of the Bering Sea, centered on specific coordinate
locations with a randomly generated polygon overlay.
...
Figure 4: A map of Alaska, based on data from worldHires in the maps package.

Notice that this map doesn't seem quite right (Figure 4), there are two ways to deal
...
(longitude is no longer negative<-small rabbit hole).
...
removes any points that are further west (east?) than 180o. Then you can create a map
with a light grey fill for the land and a black outline for the coast (Figure 5).

```{r}
ak<-map_data('worldHires','USA:Alaska')
ak<-subset(ak,long<0) #drop the end of the Aleutian Islands,can use world2Hires if need these locations

akmap<-ggplot()+geom_polygon(data=ak,aes(long,lat,group=group),fill=8,color="black")
akmap
```
Figure 5: ggplot map of Alaska based upon worldHires data.

Or we can simply constrain the data for the map if we are only interested in a particular
area of Alaska. We can also clean the map up while zooming into a particular areas (Figure 6),
in this case the western Gulf of Alaska. Note that the code to zoom in is not simply done by
changing the x and y limits. When you constrain the x and y limits on ggplot it throws out
any data outside of the image, therefore the polygon shapes ("groups") will not be complete
and the image will not be pretty. To achieve this with this plot I've incorporated the limits in
a projection wrapper (coord map), there are a number of different projections
...
Figure 6: Zoomed in ggplot map of the western Gulf of Alaska based upon worldHires data.
