...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.49\textwidth}
		<<base0, fig.align='left',echo=FALSE>>=
library(ggplot2)
ggplot(mtcars, aes(wt, mpg))+geom_point()
@
...
\end{subfigure}%
	...
	\begin{subfigure}[b]{0.49\textwidth}
   	<<base1, fig.align='left',echo=FALSE>>=
ggplot(mtcars, aes(wt, mpg))+geom_point()+theme_bw()
@
\caption{White background}
	...
	%(or a blank line to force the subfigure onto a new line)
	\begin{subfigure}[b]{0.49\textwidth}
      <<base2, fig.align='center',echo=FALSE>>=
ggplot(mtcars, aes(wt, mpg))+geom_point()+theme_bw()+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank())
@
...
<<font, out.width=".49\\textwidth",fig.align='center',fig.cap="Font change first try",fig.pos='H', eval=FALSE>>=
ggplot(mtcars, aes(wt, mpg)) + geom_point() +
...
\begin{figure}[H]
   \centering
	\begin{subfigure}[b]{0.49\textwidth}
<<font0, fig.align='left',echo=FALSE,dev='png'>>=
library(ggplot2)
ggplot(mtcars, aes(wt, mpg))+geom_point()+
  theme_bw(base_size=12)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
<<fontfake, message=FALSE, warning=FALSE,  eval=FALSE>>=
library(extrafont)
#font_import()
loadfonts(device="win")
@
...
<<update,warning=FALSE,error=FALSE,message=FALSE,dev='png'>>=
...
<<point, message=FALSE,warning=FALSE,error=FALSE, fig.height=4,fig.width=4, fig.align='center', fig.cap='Point size.',dev='png' ,dpi=300, fig.pos='H'>>=
...
<<color, message=FALSE,warning=FALSE,error=FALSE, fig.height=4,fig.width=4, fig.align='center', fig.cap='Multiple smoothers.',dev='png',dpi=300, fig.pos='H'>>=
ggplot(mtcars, aes(wt, mpg))+geom_point(aes(size=hp, color=hp))+
  geom_smooth(alpha=.1)
@

Now the colors and sizes could be determined by changing hp to a factor.
<<eval=FALSE>>=
...
However this will often create too many discrete groups to clearly understand what is being plotted, therefore a ``better" method is to define the scales for the data points. This can be done with scale\_color\_gradientn().
...
\begin{figure}[H]
	...
	%(or a blank line to force the subfigure onto a new line)
	\begin{subfigure}[b]{0.45\textwidth}
<<echo=FALSE, fig.height=4,fig.width=4, fig.align='center', fig.cap='Multiple smoothers.',dev='png',dpi=300, fig.pos='H'>>=
ggplot(mtcars, aes(wt, mpg))+geom_point(aes(size=factor(hp), color=factor(hp)))+
  geom_smooth(alpha=.1)
