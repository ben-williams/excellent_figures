...
```{r map, fig.align='center', fig.cap='Figure 18. A map of the Bering Sea, pulled directly from Google Maps via ggmap.', warning=F, message=F}
library(ggmap)
...
First call the map.
```{r ggmap1, message=FALSE, fig.height=4, fig.cap='Figure 19. A slightly zoomed in map of the Bering Sea, centered on specific coordinate locations.',dev='png',dpi=300, fig.pos='H',cache=F, warning=F}
bs1<-get_map(location = c(-170,63), zoom = 6)
ggmap(bs1)
```

Next add points to the map (Figure 19).
```{r ggmap2, message=FALSE, fig.height=4, fig.cap='Figure 20 A slightly zoomed in map of the Bering Sea, centered on specific coordinate locations with randomly generated points overlayed.',dev='png',dpi=300, fig.pos='H', cache=F, warning=F}
...
```{r ggmap3,message=FALSE, fig.height=4, fig.cap='Figure 21. A slightly zoomed in map of the Bering Sea, centered on specific coordinate locations with a randomly generated polygon overlay.',dev='png',dpi=300, fig.pos='H',cache=F, warning=F}
ggmap(bs1)+geom_point(data=locs,aes(long,lat),size=5,color=4)+
...
```{r map1, fig.height=4, fig.cap="Figure 22. A map of Alaska, based on data from worldHires in the maps package."}
library(ggplot2)
library(maps)
library(mapdata)
library(mapproj)
...
```{r map2, message=FALSE, fig.height=5, fig.cap='Figure 23. ggplot2 map of Alaska based upon worldHires data.'}
ak<-map_data('worldHires','USA:Alaska')
...
shown earlier.
```{r map3, message=FALSE, fig.height=4, fig.cap='Figure 24. ggplot2 map of Alaska based upon worldHires data all dressed up with color.', fig.pos='H',dpi=300,dev='png'}
ak<-map_data('worldHires','USA:Alaska')
ggplot()+geom_polygon(data=ak,aes(long,lat,group=group),fill=8,color="black")+
...
```{r map4,message=FALSE, fig.height=4, fig.cap='Figure 25. Zoomed in ggplot map of the western Gulf of Alaska based upon worldHires data.',dpi=300,dev='png', fig.pos='H'}
ggplot()+geom_polygon(data=ak,aes(long,lat,group=group),fill=8,color='black') +
...
```{r map5, message=FALSE, fig.height=5, fig.cap='Figure 26. ggplot2 map of Alaska based upon worldHires data. with different projections'}
ak<-map_data('worldHires','USA:Alaska')
ak<-subset(ak,long<0) #drop the end of the Aleutian Islands, or use world2Hires 
akmap<-ggplot()+geom_polygon(data=ak,aes(long,lat,group=group),fill=8,color="black")
akmap + coord_map("gilbert")
```
